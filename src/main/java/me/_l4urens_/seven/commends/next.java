...
public class next implements CommandExecutor {
    private final Map<UUID, Integer> allowedNextPage = new HashMap<>();
    @Override
    public boolean onCommand(CommandSender sender, Command command, String s, String[] args) {
        if(sender instanceof Player p){
        Integer num = Integer.parseInt(p.getScoreboard().getEntries().toArray()[0].toString());
           if(!p.getScoreboardTags().contains("can use")){p.sendMessage(ChatColor.RED+"u cant use this command now"); return false;}
